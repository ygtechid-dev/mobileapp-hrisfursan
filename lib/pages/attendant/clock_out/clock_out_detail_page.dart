...
class ClockOutDetailPage extends StatefulWidget {
  ...
  ClockOutDetailPage(this.token, this.latitude, this.longitude, this.location, this.fileImage);
...
class _ClockOutDetailPageState extends State<ClockOutDetailPage> {
      ...
      child: GeneralPage(
                ...
                Container(
                    ...
                    child: Column(
                      children: [
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  ButtonCard("retake".trans(context), (defaultWidth - 2*defaultMargin3) - 2*12, mainColor, iconData: Icons.sync, colorGradient: buttonGradient, onPressed: () async {
                                    Get.to(ClockOutCameraPage(widget.token, "${widget!.latitude}", "${widget!.longitude}", widget.location));
                                  }),
                                ]
                            )
                        ),
                        SizedBox(height: 15),
                        FormWithLabelCard(
                            outerLabelText: "clockout_notes".trans(context),
                            hintText: "clockout_notes_hint".trans(context),
                            controller: noteC,
        ...
        navBar: Container(
          ...
          child: ButtonCard("clock_out".trans(context), defaultWidth - 2*24, mainColor, isLoading: isLoading, colorGradient: buttonGradient, onPressed: () async {
            ...
            await AttendanceServices.clockOut(widget.token, widget.location, widget.latitude, widget.longitude, widget.fileImage, noteC.text).then((result) async {

              if(result != null && result.value != null){

                SharedPreferences prefs = await SharedPreferences.getInstance();
                await prefs.setString('clockout', "${result.value!.attendance!.clock_out}");
  ...
  void modalBottomSheet(contexts, String token){
    ...
    showModalBottomSheet(
        ...
        builder: (bc){
          return ModalClockOutSuccessCard(token, fullWidth, 16);
        });
  }
}
